...
module.exports = async function (io) {
  ...
  const loadMessagesWithRetry = async (
    ...
    try {
      const retries = await redisUtils.RedisUtils.getMessageRetries(retryKey);
      ...
      const currentRetries =
        (await redisUtils.RedisUtils.getMessageRetries(retryKey)) || 0;

      if (currentRetries < MAX_RETRIES) {
        await redisUtils.RedisUtils.incrementMessageRetries(retryKey);
  ...
  io.use(async (socket, next) => {
    try {
      ...
      // 이미 연결된 사용자인지 확인
      const existingSocketId = await redisUtils.RedisUtils.getConnectedSocket(
        decoded.user.id
      );
      if (existingSocketId) {
  ...
  io.on("connection", async (socket) => {
    ...
    if (socket.user) {
      // 이전 연결이 있는지 확인
      const previousSocketId = await redisUtils.RedisUtils.getConnectedSocket(
        socket.user.id
      );
      ...
      // 새로운 연결 정보 저장
      await redisUtils.RedisUtils.setConnectedSocket(socket.user.id, socket.id);
    }
    ...
    socket.on("fetchPreviousMessages", async ({ roomId, before }) => {
      ...
      try {
        ...
        const msgQueue = await redisUtils.RedisUtils.getMessageQueue(queueKey);
        if (msgQueue) {
        ...
        await redisUtils.RedisUtils.setMessageQueue(queueKey, true);
        ...
        setTimeout(async () => {
          await redisUtils.RedisUtils.removeMessageQueue(queueKey);
        }, LOAD_DELAY);
      }
    });
    ...
    socket.on("joinRoom", async (roomId) => {
      try {
        ...
        // 이미 해당 방에 참여 중인지 확인
        const currentRoom = await redisUtils.RedisUtils.getUserRoom(
          socket.user.id
        );
        ...
        if (currentRoom) {
          ...
          await redisUtils.RedisUtils.removeUserRoom(socket.user.id);
        ...
        socket.join(roomId);
        await redisUtils.RedisUtils.setUserRoom(socket.user.id, roomId);
        ...
        // 활성 스트리밍 메시지 조회
        const sessions = await redisUtils.RedisUtils.getAllStreamingSessions(
          roomId
        );
        const activeStreams = sessions.map((session) => ({
      ...
      try {
        ...
        for (const session of sessions) {
          if (session.userId === socket.user.id) {
            await redisUtils.RedisUtils.removeStreamingSession(
              session.messageId
            );
          }
        }
        ...
        const queueKey = `${roomId}:${socket.user.id}`;
        await redisUtils.RedisUtils.removeMessageQueue(queueKey);
        await redisUtils.RedisUtils.removeMessageRetries(queueKey);
    ...
    socket.on("disconnect", async (reason) => {
      ...
      try {
        ...
        if (
          (await redisUtils.RedisUtils.getConnectedSocket(socket.user.id)) ===
          socket.id
        ) {
          await redisUtils.RedisUtils.removeConnectedSocket(socket.user.id);
        }

        const roomId = await redisUtils.RedisUtils.getUserRoom(socket.user.id);
        await redisUtils.RedisUtils.removeUserRoom(socket.user.id);

        // 메시지 큐 정리
        const keys = await redisUtils.RedisUtils.getMessageQueueKeys();
        const userQueues = keys.filter((key) =>
          key.endsWith(`:${socket.user.id}`)
        );
        await Promise.all(
          userQueues.map(async (key) => {
            await redisUtils.RedisUtils.removeMessageQueue(key);
            await redisUtils.RedisUtils.removeMessageRetries(key);
          })
        );
  ...
  async function handleAIResponse(io, room, aiName, query) {
    ...
    // 스트리밍 세션 초기화
    await redisUtils.RedisUtils.setStreamingSession(messageId, {
      room,
      aiType: aiName,
      content: "",
      messageId,
    ...
    try {
      ...
      await aiService.generateResponse(query, aiName, {
        ...
        onChunk: async (chunk) => {
          ...
          const session = await redisUtils.RedisUtils.getStreamingSession(
            messageId
          );
          if (session) {
        ...
        onComplete: async (finalContent) => {
          // 스트리밍 세션 정리
          await redisUtils.RedisUtils.removeStreamingSession(messageId);
